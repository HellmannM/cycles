...
template<typename T> static void zeroOutStruct(T &v)
{
#if 0
...
Array::Array(ANARIDataType type, CyclesGlobalState *state, const ArrayMemoryDescriptor &d)
    : helium::BaseArray(type, state), m_elementType(d.elementType)
{
  state->objectCounts.arrays++;

  if (d.appMemory) {
    m_ownership = d.deleter ? ArrayDataOwnership::CAPTURED : ArrayDataOwnership::SHARED;
  }
  else
...
void *Array::data() const
{
  switch (ownership()) {
    case ArrayDataOwnership::SHARED:
      return const_cast<void *>(wasPrivatized() ? m_hostData.privatized.mem :
                                                  m_hostData.shared.mem);
      break;
    case ArrayDataOwnership::CAPTURED:
...
{
  if (m_mapped) {
    reportMessage(ANARI_SEVERITY_WARNING, "array mapped again without being previously unmapped");
  }
  m_mapped = true;
  return data();
}
...
{
  if (!m_mapped) {
    reportMessage(ANARI_SEVERITY_WARNING, "array unmapped again without being previously mapped");
    return;
  }
  m_mapped = false;
...
CyclesGlobalState *Array::deviceState() const
{
...
CYCLES_ANARI_TYPEFOR_DEFINITION(cycles::Array *);
