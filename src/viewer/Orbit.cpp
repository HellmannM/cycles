...
static float3 azelToDirection(float az, float el, OrbitAxis axis)
{
  ...
  case OrbitAxis::POS_X:
    return -normalize(make_float3(z, y, x));
  case OrbitAxis::POS_Y:
    return -normalize(make_float3(x, z, y));
...
float2 Orbit::azel() const
{
  return m_azel;
}

float3 Orbit::eye() const
{
  return m_eye;
}

float3 Orbit::dir() const
{
  return normalize(m_at - m_eye);
}

float3 Orbit::at() const
{
  return m_at;
}

float3 Orbit::up() const
{
  return m_up;
}
...
float3 Orbit::eye_FixedDistance() const
{
